...
def create_polymer_Ucg(msm_savedir, n_beads, M, beta, fix_back, fix_exvol,
        ...
        bond_cutoff, cv_lin_pot=None, a_coeff=None, n_cvs=None, pair_symmetry=None):
    ...
    if using_cv:
        ...
        if (cv_lin_pot is None) or (cv_lin_pot == False):
            ...
            Ucg.linear_cv_potential()
            Ucg.using_linear = True
        Ucg.gaussian_cv_potentials(cv_r0_basis, cv_w_basis)
    else:
        print("  parametric pairwise potentials")
        cv_r0_basis = None
        gauss_r0_nm = np.linspace(0.3, 1, n_pair_gauss)
        gauss_sigma = gauss_r0_nm[1] - gauss_r0_nm[0]
        gauss_w_nm = 2*gauss_sigma*np.ones(len(gauss_r0_nm))
        Ucg.gaussian_pair_potentials(gauss_r0_nm, gauss_w_nm, scale_factor=1,
                symmetry=pair_symmetry)

        # cartesian test functions, didn't make sense.
